 document.addEventListener('DOMContentLoaded', () => {
            ...
            const fetchFiles = async () => {
                ...
                filesList.innerHTML = '';
                if (files.length === 0) {
                    filesList.innerHTML = '<p class="text-white py-4">Anda belum mengunggah file.</p>';
                    return;
                }
                files.forEach(file => {
                    const fileItem = document.createElement('div');
                    fileItem.classList.add('glass-file-item', 'p-4', 'rounded-lg', 'flex', 'flex-col', 'sm:flex-row', 'justify-between', 'items-start', 'sm:items-center', 'gap-3');
                    
                    const fileInfo = document.createElement('div');
                    fileInfo.classList.add('flex', 'flex-col');
                    ...
                    fileDate.classList.add('file-date', 'mt-1');
                    fileDate.textContent = `Diunggah: ${date}`;

                    const fileActions = document.createElement('div');
                    fileActions.classList.add('flex', 'gap-2', 'mt-2', 'sm:mt-0');
